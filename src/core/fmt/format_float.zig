...
pub fn formatFloat(sink: *io.Sink, v_: anytype, options: FormatOptions) io.Sink.WriteError!void {
    ...
    const T = @TypeOf(v);
    comptime debug.assert(@typeInfo(T) == .float);
    ...
    const has_explicit_leading_bit = math.floatMantissaBits(T) - math.floatFractionalBits(T) != 0;
    const d = binaryToDecimal(DT, @as(I, @bitCast(v)), math.floatMantissaBits(T), math.floatExponentBits(T), has_explicit_leading_bit, tables);

    return switch (options.mode) {
        .scientific => formatScientific(DT, sink, d, options.precision),
        .decimal => formatDecimal(DT, sink, d, options.precision),
    };
}

pub fn FloatDecimal(comptime T: type) type {
    comptime debug.assert(T == u64 or T == u128);
    return struct {
        mantissa: T,
        exponent: i32,
        sign: bool,
...
fn writeSpecialStr(sink: *io.Sink, f: anytype) io.Sink.WriteError!void {
    if (f.sign) {
        try sink.writeByte('-');
    }

    if (f.mantissa != 0) {
        try sink.writeAll("nan");
    } else {
        try sink.writeAll("inf");
    }
}

fn writeDecimal(sink: *io.Sink, value: anytype, count: usize) io.Sink.WriteError!void {
    var i: usize = count;

    while (i > 2) : (i -= 2) {
        // Get remainder, 12345 -> 2345
        const remainder = value.* % math.pow(u128, 10, i);
        // Get the digit, 2345 -> 23
        const digit: u8 = @intCast(remainder / math.pow(u128, 10, i - 2));
        const d = fmt.digits2(digit);
        try sink.writeAll(&d);
    }
    ...
    while (i > 0) : (i -= 1) {
        ...
        const remainder = value.* % math.pow(u128, 10, i);
        // Get the digit, 2345 -> 23
        const digit: u8 = @intCast(remainder / math.pow(u128, 10, i - 1));
        try sink.writeByte('0' + digit);
    }
}
...
pub fn formatScientific(comptime T: type, sink: *io.Sink, f_: FloatDecimal(T), precision: ?usize) io.Sink.WriteError!void {
    var f = f_;

    if (f.exponent == special_exponent) {
        return writeSpecialStr(sink, f);
    }
    ...
    // 1.12345
    try sink.writeByte('0' + @as(u8, @intCast(output % math.pow(u128, 10, olength) / math.pow(u128, 10, olength - 1))));
    if (olength > 1) {
        try sink.writeByte('.');
    }
    try writeDecimal(sink, &output, olength - 1);

    if (precision) |prec| {
        if (prec > olength - 1) {
            const len = prec - (olength - 1);
            try sink.writeByteNTimes('0', len);
        }
    }

    // e100
    try sink.writeByte('e');
    ...
    try writeDecimal(sink, &uexp, elength);
}

/// Write a FloatDecimal to a sink in decimal form.
pub fn formatDecimal(comptime T: type, sink: *io.Sink, f_: FloatDecimal(T), precision: ?usize) io.Sink.WriteError!void {
    var f = f_;
    ...
    if (dp_offset <= 0) {
        // 0.000001234
        try sink.writeAll("0.");
        const dp_index = sink.pos;

        const dp_poffset: u32 = @intCast(-dp_offset);
        try sink.writeByteNTimes('0', dp_poffset);
        try writeDecimal(sink, &output, olength);

        if (precision) |prec| {
            const dp_written = sink.pos - dp_index;
            if (prec > dp_written) {
                try sink.writeByteNTimes('0', prec - dp_written);
            }
        }
    } else {
        // 123456000
        ...
        if (dp_uoffset >= olength) {
            try writeDecimal(sink, &output, olength);
            try sink.writeByteNTimes('0', dp_uoffset - olength);

            if (precision) |prec| {
                if (prec != 0) {
                    try sink.writeByte('.');
                    try sink.writeByteNTimes('0', prec);
                }
            }
        } else {
            // 12345.6789
            try writeDecimal(sink, &output, dp_uoffset);
            try writeDecimal(sink, &output, olength - dp_uoffset);
...
pub fn binaryToDecimal(comptime T: type, bits: T, mantissa_bits: math.Log2Int(T), exponent_bits: u5, explicit_leading_bit: bool, comptime tables: anytype) FloatDecimal(T) {
    if (T != tables.T) {
...
fn decimalLength(v: anytype) u32 {
    switch (@TypeOf(v)) {
        u32, u64 => {
            debug.assert(v < 100000000000000000);
...
fn multipleOfPowerOf2(value: anytype, p: u32) bool {
    const T = @TypeOf(value);
    debug.assert(@typeInfo(T) == .int);
    return (value & ((@as(T, 1) << @as(math.Log2Int(T), @intCast(p))) - 1)) == 0;
}
...
fn check(comptime T: type, value: T, comptime expected: []const u8) !void {
    ...
    var sink = io.Sink.fixed(&buf);
    try formatFloat(&sink, value, .{});
    try expect(sink.buffered()).toEqual(expected);

    // if (T == f80 and builtin.target.os.tag == .windows and builtin.target.cpu.arch == .x86_64) return;
    //
    // const o = try fmt.parseFloat(T, sink.buffered());
    // const o_bits: I = @bitCast(o);
    //
    // if (math.isNan(value)) {
    //     try expect(math.isNan(o)).toEqual(true);
    // } else {
    //     try expect(o_bits).toEqual(value_bits);
    // }
}
...
test "format f32" {
    ...
    try check(f32, -1.0, "-1e0");
    try check(f32, math.nan(f32), "nan");
    try check(f32, math.inf(f32), "inf");
    try check(f32, -math.inf(f32), "-inf");
...
test "format f64" {
    ...
    try check(f64, -1.0, "-1e0");
    try check(f64, math.nan(f64), "nan");
    try check(f64, math.inf(f64), "inf");
    try check(f64, -math.inf(f64), "-inf");
...
test "format f80" {
    ...
    try check(f80, -1.0, "-1e0");
    try check(f80, math.nan(f80), "nan");
    try check(f80, math.inf(f80), "inf");
    try check(f80, -math.inf(f80), "-inf");
...
test "format f128" {
    ...
    try check(f128, -1.0, "-1e0");
    try check(f128, math.nan(f128), "nan");
    try check(f128, math.inf(f128), "inf");
    try check(f128, -math.inf(f128), "-inf");
...
// var fmt_buf: [6000]u8 = undefined;
// fn expectFmt(comptime format: []const u8, args: anytype) Expect([]u8) {
//     var sink = io.Sink.fixed(&fmt_buf);
//     try fmt.format(&sink, format, args);
//     return expect(sink.buffered());
// }
//
// test "format float to decimal with zero precision" {
//     try expectFmt("{d:.0}", .{5}).toEqual("5");
//     try expectFmt("{d:.0}", .{6}).toEqual("6");
//     try expectFmt("{d:.0}", .{7}).toEqual("7");
//     try expectFmt("{d:.0}", .{8}).toEqual("8");
// }
